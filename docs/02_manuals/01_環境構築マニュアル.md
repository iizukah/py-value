...
本番環境へのデプロイ手順は、利用するプラットフォーム（Firebase Hosting / Vercel / Cloud Run 等）に依存する。詳細は以下を参照する。

- **運用・デプロイの仕様**: `docs/01_specs/06_operations/INFRA-01_operations_and_deploy.md`
- **環境変数・シークレット**: プロジェクト内 **reference.md**（インフラ担当が整備）に、本番用の設定場所と手順が記載される。

一般的な流れ:

1. 本番用の環境変数・シークレットをプラットフォームの画面で設定する。
2. Firestore 等のデータソースを用意し、必要ならシードデータを投入する。
