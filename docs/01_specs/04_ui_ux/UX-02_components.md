...
|------|------|
| **ID** | UX-02 |
| **関連ドキュメント** | REQ-01, REQ-02, UX-01, ARC-02, UX-03（プラグイン別は plugins/ 参照） |

本ドキュメントは枠組みの共通コンポーネント ID（CP-001～）、テーマ・トークン（推奨）、アクセシビリティ（NFR-F004 の具体）、エラー表示・文言方針（FR-P011）を定義する。プラグイン別のコンポーネント詳細は [04_ui_ux/plugins/UX-03_plugin_python_analysis.md](plugins/UX-03_plugin_python_analysis.md) を参照する。

---

## 2. 枠組みの共通コンポーネント

各コンポーネントに **CP-001 ～ CP-00x** を付与する。役割と UX-01 の画面（SC-xxx）との対応を記載する。

| コンポーネント ID | 名称 | 役割 | 主な利用画面 |
|-------------------|------|------|--------------|
| **CP-001** | レイアウト（共通） | ヘッダ・パンくず・メインコンテンツ領域を配置する共通レイアウト。 | 全画面 |
| **CP-002** | パンくず | URL パスから階層構造を表示。動的ルーティングと連動（FR-F001）。 | SC-001, SC-002 等 |
| **CP-003** | 問題カード | 問題一覧の 1 件表示。タイトル・難易度・バッジ・リンク。 | SC-001 |
| **CP-004** | 問題一覧コンテナ | ソート切り替え UI と問題カードリスト。0 件時メッセージ表示。 | SC-001 |
...
上記はあくまで推奨であり、プロジェクト方針に応じて変更してよい。
...
- **キーボード操作**: 主要な操作（問題選択、下書き保存、解答送信、ソート切り替え、管理画面の CRUD）はキーボードのみで実行可能とする。フォーカス順序は論理的であること。
...
- REQ-01 FR-F002, FR-F009, FR-F018, NFR-F004
- REQ-02 FR-P011（エラー種別）
- UX-01 画面定義（SC-001～SC-010）
