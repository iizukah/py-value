...
|-------------------|------|------|--------------|
| **CP-000** | ファーストビュー | ホームの訴求エリア。キャッチコピー「Exercise the Mind, Master the Skill.」、メイン CTA「GET STARTED」のみ表示。画像を使わず CSS・JavaScript のみ（FR-F023）。見た目は [reference/mock.html](reference/mock.html) に準拠。 | SC-000 |
| **CP-001** | レイアウト（共通） | ヘッダ（ロゴ SVG・ナビ：問題一覧・お気に入り・履歴・管理）、パンくず・メインコンテンツ領域、フッター（SYSTEM ONLINE／EXER FRAMEWORK）を配置する共通レイアウト。見た目は [reference/mock.html](reference/mock.html) を参照。 | 全画面 |
...
| **CP-015** | 問題集選択 | ワークブック一覧をカードで表示（Data Analysis／SQL Expert Coming Soon／Statistics 等）。選択後は当該ワークブックの問題一覧へ遷移（FR-F024）。見た目は [reference/mock.html](reference/mock.html) に準拠。 | SC-000 |
...
| **CP-009** | 採点結果表示ブロック | 採点依頼後、約 2 秒のローディング表示ののち、バッジ（Passed/Fail）＋ラベル＋結果タイルで正誤・メッセージ・詳細を表示。SC-003 の構成要素。見た目は [reference/mock.html](reference/mock.html) に準拠。 | SC-002, SC-003 |
