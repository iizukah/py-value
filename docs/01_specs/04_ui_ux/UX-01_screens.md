...
本ドキュメントは枠組みの画面一覧・画面 ID（SC-000～SC-011）、画面遷移、各画面の役割・主要要素を定義する。**パンくず・ナビ**では、ワークブック ID `py-value` の表示名は **「Pythonデータ分析」** とする（REQ-01 用語定義）。**見た目（UI の具体）**は [reference/mock.html](reference/mock.html) を正とする。**ワイヤーフレーム**は [wireframes/wireframes.html](wireframes/wireframes.html) を補助参照として利用する。
...
- **主要 UI 要素**: ファーストビュー（CP-000）にキャッチコピー「Exercise the Mind, Master the Skill.」、メイン CTA「GET STARTED」、問題集カード（CP-015：Data Analysis／SQL Expert Coming Soon／Statistics 等）。ロゴ（SVG）・ファビコン、背景アニメーション、フッター（SYSTEM ONLINE／EXER FRAMEWORK）を配置。画像なし・CSS/JavaScript のみ。ルート `/` で表示。パンくずの先頭は「ホーム」。
- **ルート**: `app/`（ルート `/`）。ARC-01-002 と整合。
- **参照**: [reference/mock.html](reference/mock.html) のホーム／ファーストビューを正とする。

### SC-001 問題一覧

- **役割**: 学習者がワークブック内の問題一覧・ステータス（バッジ・タグ・お気に入り数含む）を確認し、問題を選択してワークスペースへ遷移する。タグフィルター・お気に入り数ソートに対応（FR-F026, FR-F028）。
- **主要 UI 要素**: タイルグリッドで問題カード一覧を表示。ソート切り替え（登録順・難易度・タイトル・お気に入り数）、タグフィルター（CP-016）、問題カード（タイトル・難易度・バッジ・タグ・お気に入りボタン・お気に入り数）、0 件時のメッセージ。履歴・お気に入りへの導線は**ヘッダーナビ**から遷移する（一覧画面下部の「お気に入り」「履歴へ」リンクは設けない）。
- **ルート**: `app/[workbookId]/`（例: `/py-value`）。パンくずでは workbookId の表示名を用いる（例: 「Pythonデータ分析」）。ARC-01-002 と整合。
- **参照**: [reference/mock.html](reference/mock.html) の問題一覧（タイルグリッド・ソート・タグ）を正とする。
...
- **役割**: 採点依頼後に「採点中」ローディングを約 2 秒表示したのち、合格/不合格と付与バッジを表示する。ワークスペース内のブロックで表示。レイアウトは画面中央に配置する。
- **主要 UI 要素**: 採点中はローディング表示（約 2 秒）→ バッジ（Passed/Fail）＋結果ラベル、結果タイル（詳細）。操作として「Retry」「一覧へ戻る」ボタン。全問正解時は完了画面（SC-004）への導線。タイル幅・Retry の配置等は今後の改善で調整可能とする。
- **参照**: [reference/mock.html](reference/mock.html) の採点結果ブロックを正とする。
...
- **主要 UI 要素**: 履歴リストのパネル構成（問題タイトル・結果・日時）、詳細へのリンク。見た目は reference/mock.html に合わせる。
- **参照**: [reference/mock.html](reference/mock.html) の履歴一覧パネル構成を正とする。

### SC-006 履歴詳細

- **役割**: 単一の解答履歴の詳細（送信内容・結果・採点結果・日時）を表示する。結果は「合格」「不合格」で表記する。
- **主要 UI 要素**: 履歴詳細のパネル構成（問題情報、userAnswer の要約、結果、judgedAt、一覧へ戻る）。見た目は reference/mock.html に合わせる。
- **参照**: [reference/mock.html](reference/mock.html) の履歴詳細パネル構成を正とする。
...
- **主要 UI 要素**: お気に入り一覧コンテナ（CP-018）のパネル構成。各問題カードに挑戦・お気に入り解除ボタン、一覧へ戻る。見た目は reference/mock.html に合わせる。
- **ルート**: `app/[workbookId]/favorites`（例: `/py-value/favorites`）。パンくずは「ホーム > Pythonデータ分析 > お気に入り」等。
- **参照**: [reference/mock.html](reference/mock.html) のお気に入り一覧パネル構成を正とする。
