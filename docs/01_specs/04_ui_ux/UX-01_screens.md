...
本ドキュメントは枠組みの画面一覧・画面 ID（SC-001～）、画面遷移、各画面の役割・主要要素を定義する。ワイヤーフレーム画像は [wireframes/](wireframes/) に配置し、本ドキュメントから相対パスで参照する。
...
- **ワイヤーフレーム**: [wireframes/screens/SC-001_list.png](wireframes/screens/SC-001_list.png)（用意次第で参照）。
...
- **ワイヤーフレーム**: プラグイン別は [wireframes/plugins/python-analysis/](wireframes/plugins/python-analysis/) を UX-03 で参照。

### SC-003 正解・バッジ表示

- **役割**: 採点成功時に正解であることと、付与されたバッジを表示する。ワークスペース内のブロックまたはモーダルで表示。
- **主要 UI 要素**: 正解メッセージ、バッジ表示、「次の問題へ」「一覧へ戻る」ボタン。全問正解時は完了画面（SC-004）への導線。

### SC-004 完了画面

- **役割**: ワークブック内の全問題に正解した状態で表示し、学習者に完了を伝える。
- **主要 UI 要素**: 完了メッセージ、一覧へ戻るボタン。
- **ルート**: `app/[workbookId]/complete` 等。

### SC-005 履歴一覧

- **役割**: 自分の解答履歴を問題単位で一覧表示する。直近 N 件（ワークブック設定に従う）。
- **主要 UI 要素**: 履歴リスト（問題タイトル・正誤・日時）、詳細へのリンク。

### SC-006 履歴詳細

- **役割**: 単一の解答履歴の詳細（送信内容・正誤・採点結果・日時）を表示する。
- **主要 UI 要素**: 問題情報、userAnswer の要約、正誤、judgedAt、一覧へ戻る。

### SC-007 管理ダッシュボード

- **役割**: 問題 CRUD、インポート/エクスポート、データセットアップロードへの入口。URL クエリの簡易キーでアクセス制御。
- **主要 UI 要素**: 問題一覧（下書き含む）、新規作成・編集・削除、インポート/エクスポート・データセットへのリンク。
