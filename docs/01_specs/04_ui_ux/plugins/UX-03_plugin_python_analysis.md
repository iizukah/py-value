# UX-03 データ分析プラグイン 画面・コンポーネント（EXER 第一版）
...
| **ID** | UX-03 |
| **プロジェクト名** | EXER |
| **関連ドキュメント** | REQ-02（データ分析プラグイン）、ARC-02, UX-01（SC-002 ワークスペース）, UX-02, UX-04, DATA-02 |

本ドキュメントはデータ分析プラグイン（type: `python-analysis`）の画面・コンポーネント構成を定義する。枠組みのワークスペース（SC-002）内に、本プラグインの 3 ペインおよび各エリアが描画される。**デザイン依存**: プラグインは枠組みの色・フォント・コンポーネント ID に直接依存せず、SC-002 のコンテナ・ルーティングおよび CP-007/CP-008/CP-009 の配置方針のみに依存する。見た目の一貫性のため、**枠組みの UX-04 デザイントークン（CSS 変数、例: `var(--color-bg-main)`）の参照は可能**とする。具体的な色・余白のハードコードは避ける。**ワイヤーフレーム**は [../wireframes/wireframes.html](../wireframes/wireframes.html) をブラウザで開き、タブ「SC-002 ワークスペース」を選択した状態を参照する。
