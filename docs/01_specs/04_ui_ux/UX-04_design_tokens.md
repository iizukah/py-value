# UX-04 デザイン仕様（トークン・アイコン）（EXER 第一版）
...
## 6. DS-006 ブランド資産（ロゴ・ファビコン）

| 項目 | 仕様 |
|------|------|
| **ロゴ** | EXER の表記。SVG で表現する場合の参考: 線（path）・円（circle）・テキスト "EXER"。フォントは **Space Grotesk**（`700`）、アクセント色（`--color-accent-emerald`）。オプションでグロー効果（`filter: drop-shadow(0 0 10px rgba(16, 185, 129, 0.5))`）。**ヘッダー**および**ホームのファーストビュー**で使用する。 |
| **ファビコン** | サイズ 32×32px。角丸（`rx="6"`）の矩形背景に、緑（`#10b981`）の横線 3 本と右端の円のモチーフ。data URI（`data:image/svg+xml,...`）または同デザインの静的 SVG ファイルを用意する。実装では `<link rel="icon" type="image/svg+xml" href="...">` で参照する。 |

---

## 7. DS-005 コンポーネント別レイアウト値
