...
## 8. 99_diff 統合（01_2025-02-08）に伴う追記

### 8.1 単体テスト（追記）

| テストケース ID | 対象 | 対応要件 ID / 差分 ID | 概要 |
|-----------------|------|------------------------|------|
| TC-016 | API Route Handler（下書き一覧） | API-021, DD-009 | GET /api/workbooks/:id/drafts が X-Client-Id で下書きあり questionId 一覧を返す。 |
| TC-P08 | セル単位 Run（Jupyter 相当） | FR-P001, DD-010 | セル単位で実行し、実行結果を Variable Watcher とセル下に表示。状態はセル間で共有される。 |
| TC-P09 | Pyodide プリロード・ローディング表示 | DD-023 | mount 時に Pyodide をプリロードし、完了まで全画面オーバーレイを表示する。 |
| TC-P10 | 採点結果時の問題一覧表示 | DD-021, DD-026 | 採点結果表示時に中央ペインでランダム最大10問を表示し、Retry で非表示になる。 |

### 8.2 結合テスト（追記）

| テストケース ID | 対応 | 概要 |
|-----------------|------|------|
| TC-INT-14 | API-021, FR-F005 | GET drafts で下書きあり questionId 一覧が返り、問題一覧からの下書き導線と整合する。 |

### 8.3 E2E テスト（追記）

| テストケース ID | 対応 UC / 画面 / 差分 ID | 概要 |
|-----------------|--------------------------|------|
| TC-E2E-14 | SC-002, DD-010, DD-022 | ワークスペースで複数セル・セル単位 Run が動作し、Variable Watcher とセル下に結果が表示される。Monaco によるコードハイライトが表示される。 |
| TC-E2E-15 | SC-002, SC-003, DD-021, DD-026 | 採点結果表示時、左ペインに採点結果・右ペインに Variable Watcher/Result Plot、中央ペインに「次の問題に挑戦！」と問題一覧が表示される。 |
| TC-E2E-16 | SC-002, DD-023 | 問題ページ遷移時、Pyodide ローディング完了までオーバーレイが表示され、完了後にワークスペースが表示される。 |

### 8.4 変更履歴

| 日付 | 変更内容 | 関連 ID |
|------|----------|---------|
| 2025-02-08 | 99_diff 統合（01_2025-02-08）に伴い、§8 を追加。API-021（下書き一覧）、DD-010（セル単位 Run）、DD-021/026（採点結果時問題一覧）、DD-022（Monaco）、DD-023（Pyodide ローディング）に対応するテストケース TC-016, TC-P08～TC-P10, TC-INT-14, TC-E2E-14～TC-E2E-16 を追記。 | 01_2025-02-08, TEST-01 |
